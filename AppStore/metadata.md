# Cherry Battle — App Store metadata

_Brand name is **Cherry Battle** (matches the app's on-device display name and the website).
Internal project/target/scheme remain `StarBattle`._

## What's New (2.4.2, ≤4000)
Smoother taps, sound in Mark mode, and a fresh board every time.

• Smoother board — placing pieces and sweeping dots now keeps up with your finger, with none of the lag that could creep in as you played.
• Mark mode has sound — sketching guesses now plays the same sound effects and taps as the real board, a little softer.
• A fresh start — four times as many opening boards, so the app no longer greets you with the same puzzle, and the board you land on first is always a gentle one.
• Polish and fixes throughout.

Thanks for playing!

## What's New (2.4.1, ≤4000) — NEVER RELEASED
_Prepared but never submitted; superseded by 2.4.2, whose notes fold in these two items.
Users went 2.4 → 2.4.2, so don't treat this as shipped._


Smoother taps, and sound in Mark mode.

• Smoother board — placing pieces and sweeping dots now keeps up with your finger, with none of the lag that could creep in as you played.
• Mark mode has sound — sketching guesses now plays the same sound effects and taps as the real board, a little softer.
• Polish and fixes throughout.

Thanks for playing!

## What's New (2.4, ≤4000)
A bigger board, and taps that go where you meant.

• A bigger board — the grid now fills the full width of the screen, so every square is larger and easier to hit.
• Clearer grid — slightly darker lines make it easier to follow a row or column across the board.
• More forgiving taps — a tap that lands near the edge of a square now goes to the square you meant, and won't knock out a piece you've already placed.
• Polish and fixes throughout.

Thanks for playing!

## What's New (2.3.1, ≤4000)
Smoother marking, and a friendlier Check.

• Smarter dot-drawing — when you swipe to sweep dots down a row or column, the line now snaps onto the one you meant: a row, column or region that already has all its pieces. No more fighting a stroke that lands one line off.
• A nicer "all clear" — pressing Check on a clean board (and the press-and-hold deep Check) now always gives a pleasant little confirming buzz.
• Polish and fixes throughout.

Thanks for playing! 🍒

## What's New (2.3, ≤4000)
Sound, motion, and live mistake-spotting.

• Sound effects — satisfying taps as you place pieces and dots, a confirming chime when you "Do it", and a cheerful jingle with pieces popping like fireworks when you win. Turn it off anytime in Settings.
• Playful pieces — an optional fun little hop-and-wiggle animation for the pieces on your board.
• Spot mistakes live — a new optional mode that flags rule-breaking pieces the moment you place them (two touching, or too many in a row, column or region) with an animated highlight and a buzz.
• Polish and fixes throughout.

Thanks for playing! 🍒

## What's New (2.2.1, ≤4000)
A tougher new Expert level, and hints that teach you the techniques.

• New Expert level — the hardest boards yet, demanding the toughest logical technique again and again. Five levels now: Beginner, Easy, Medium, Hard and Expert.
• Hints that teach — every hint now names the solving technique it used, with a tap-through explanation of when and why that technique works, so you build real skills instead of just getting the answer.
• Polish and fixes throughout.

Thanks for playing! 🍒

## What's New (1.2, ≤4000)
Pick your challenge, track your progress, and play in your language.

• Three difficulty levels — Easy, Medium and Hard, graded by how much step-by-step logic a solve really needs. Hard now demands genuinely deeper (but still 100% logical) reasoning.
• Get rewarded — string together five clean solves and the game offers to step you up.
• Per-level stats — games solved, solve rate, best & average times, hints used and mistakes caught, tracked separately for each difficulty.
• Smarter hints — they now reason through the harder boards too, and always explain why.
• Simpler Mark mode — it works just like the real board (tap for a dot, tap again for a cherry), with full-size Undo/Redo/Erase/Do-it buttons and a new in-app guide.
• Localized for 10 languages, a refined new title, and a fresh look.

## App name (≤30)
Cherry Battle

## Subtitle (≤30)
Two-not-touch logic puzzle

## Promotional text (≤170)
Two cherries in every row, column and region — and never touching. Five difficulties up to a brutal new Expert, plus hints that teach you the techniques. Free to play.

## Keywords (≤100, comma-separated)
logic puzzle,star battle,two not touch,brain,queens,deduction,grid,nonogram,sudoku,offline,puzzle

## Description (≤4000)
Cherry Battle is a clean, friendly take on Star Battle — the elegant "two-not-touch" logic puzzle.

The rules are simple, the deductions are deep: place two cherries in every row, every column and every coloured region. The catch — no two cherries may ever touch, not even diagonally. From those few rules grows a surprisingly rich puzzle you solve with pure logic.

Every board is generated fresh on your device with exactly one solution, and the game favours puzzles you can crack by reasoning alone — so you never have to guess.

WHY YOU'LL LIKE IT
• Five difficulties — a gentle Beginner board to learn on, then Easy, Medium, Hard, and a brutal new Expert that leans on the hardest technique again and again.
• Hints that teach — stuck on a square? A hint places the move logic forces, names the technique behind it, and explains when and why that technique works — so you learn new tactics instead of just getting the answer.
• Mark mode — try a "what if" on a scratch layer: pencil in a line of candidate guesses, follow the logic, then commit them in one satisfying sweep.
• Smart helpers — placing a cherry automatically dots its neighbours, drag to sweep dots down a line, and Undo or Redo any move.
• Share a board — send a friend a link that opens the exact same puzzle to race or compare.
• Track your progress — games solved, solve rate, best & average times, hints used and mistakes caught, per difficulty.
• Make it yours — a light or dark theme, a quiet timer you can hide, and a playing piece you pick: cherry, star, robot, unicorn, dino and more.
• Make it playful — optional sound effects, a fun hop-and-wiggle piece animation, and a live mistake-checker that flags rule-breaking pieces the moment you place them.
• iPhone, iPad & Mac — an adaptive board that fills the screen on any device, in portrait or landscape.
• Sweet victory — solve it and the board bursts into a shower of pieces and a celebratory rumble of haptics.
• No ads, no accounts, no tracking — and it works completely offline.

FREE TO PLAY
Beginner mode is always free and unlimited, and Easy, Medium, Hard and Expert each give you a fresh puzzle every day at no cost. Love it? A single one-time purchase unlocks unlimited new puzzles in every mode, forever — no subscriptions.

Whether you have two minutes in a queue or twenty on the couch, Cherry Battle is a calm, rewarding little brain workout.

Cherry Battle is a friendly take on Star Battle, the classic logic puzzle invented by Hans Eendebak. All credit for the original puzzle design goes to him.

## Support / Marketing URL
https://jiroikedafeingold.github.io/StarBattle/

## Notes
- The marketing/support URL is this repo's GitHub Pages site (served from `/docs` on
  `main`). It only goes live once GitHub Pages is enabled for the repo:
  Settings → Pages → Source: "Deploy from a branch" → Branch `main`, folder `/docs`.
- Keep the description honest: puzzles are **generated on device**, not hand-authored.
- The App Store listing name is **Cherry Battle**; the Xcode target/scheme is still `StarBattle`.
