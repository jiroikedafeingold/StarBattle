...
/// The game title: "STAR BATTLE+" in all caps to echo the star key-art — a golden "STAR",
/// a royal-blue "BATTLE" and a golden "+". Solid colours with only a hint of shading — a
...
struct GameTitle: View {
    var body: some View {
        HStack(spacing: 2) {
            ...
            ShadedWord(text: "+", color: Color(hex: 0xF3A81A), shade: Color(hex: 0xB5790A))
        }
...
/// One title word: a solid colour with a slightly darker copy nudged just beneath it, for
/// a gentle sense of depth without any gradient or gloss.
private struct ShadedWord: View {
    let text: String
    let color: Color
    let shade: Color
    ...
    var body: some View {
        ZStack {
            Text(verbatim: text).foregroundStyle(shade).offset(y: 1)
